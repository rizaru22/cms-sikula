...
class SitemapController extends Controller
{
    ...
    public function news()
    {
        $xml=Cache::remember('sitemap-news',3600,function(){
            $items=\App\Models\News::select('slug','updated_at')->latest('updated_at')->get();
            $baseUrl=url('/berita');
    ...
    public function prestasi()
    {
        $xml=Cache::remember('sitemap-prestasi',3600,function(){
            $items=\App\Models\Achievement::select('slug','updated_at')->latest('updated_at')->get();
            $baseUrl=url('/prestasi');
            return view('sitemap.partials.urlset',compact('items','baseUrl'))->render();
    ...
    public function produk()
    {
        $xml=Cache::remember('sitemap-produk',3600,function(){
            $items=\App\Models\Produk::select('slug','updated_at')->latest('updated_at')->get();
            $baseUrl=url('/produk');
            return view('sitemap.partials.urlset',compact('items','baseUrl'))->render();
    ...
    public function kompetensi()
    {
        $xml=Cache::remember('sitemap-kompetensi',3600,function(){
            $items=\App\Models\KompetensiKeahlian::select('slug','updated_at')->latest('updated_at')->get();
